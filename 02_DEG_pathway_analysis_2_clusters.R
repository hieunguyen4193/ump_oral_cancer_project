...
for (cluster.name in all.names.2clusters){
  ...
  dir.create(file.path(path.to.02.output, "pathway_results_ORA"), showWarnings = FALSE, recursive = TRUE)
  dir.create(file.path(path.to.02.output, "pathway_plots_ORA"), showWarnings = FALSE, recursive = TRUE)
  dir.create(file.path(path.to.02.output, "pathway_objs_ORA"), showWarnings = FALSE, recursive = TRUE)
  
  dir.create(file.path(path.to.02.output, "pathway_results_GSEA"), showWarnings = FALSE, recursive = TRUE)
  dir.create(file.path(path.to.02.output, "pathway_plots_GSEA"), showWarnings = FALSE, recursive = TRUE)
  dir.create(file.path(path.to.02.output, "pathway_objs_GSEA"), showWarnings = FALSE, recursive = TRUE)
  
  dir.create(file.path(path.to.02.output, "count_matrix"), showWarnings = FALSE, recursive = TRUE)
  
  writexl::write_xlsx(as.data.frame(config.params[[code.version]]), file.path(path.to.02.output, "config.params.logs.xlsx"))
  ...
  saveRDS(dds.obj, file.path(path.to.02.output, "DESEQ2_RNAseq_data_one_vs_one.obj.rds"))
  ...
  for (i in names(dds.obj)){
    ...
    write.table(save.count.matrix, file.path(path.to.02.output, "count_matrix", sprintf("count_matrix_%s.csv", i)), sep = "\t")
    write.table(phenotypedf, file.path(path.to.02.output, "count_matrix", sprintf("phenotype_%s.csv", i)), sep = "\t")
  }
  ...
  for (i in names(dds.obj)){
    ...
    writexl::write_xlsx(tmpdf, file.path(path.to.02.output, "DESEQ_results", sprintf("DESEQ_result_%s.xlsx", i)))
  ...
  generate_results_from_enricher_ORA <- function(cluster1, cluster2){
    ...
    saveRDS(output, file.path(path.to.02.output, "pathway_objs_ORA", sprintf("pathway_obj_cluster%s_vs_cluster%s.rds", cluster1, cluster2)))
    for (i in names(output$dfs)){
      tmp.table <- output$dfs[[i]]
      if (nrow(tmp.table) != 0){
        print(sprintf("Saving results from pathway %s", i))
        writexl::write_xlsx(tmp.table, file.path(path.to.02.output, "pathway_results_ORA", sprintf("enriched_pathway_%s.xlsx", i)))
      }
    }
  ...
  generate_results_from_enricher_GSEA <- function(cluster1, cluster2){
    ...
    saveRDS(output, file.path(path.to.02.output, "pathway_objs_GSEA", sprintf("pathway_obj_cluster%s_vs_cluster%s.rds", cluster1, cluster2)))
    for (i in names(output$dfs)){
      tmp.table <- output$dfs[[i]]
      if (nrow(tmp.table) != 0){
        print(sprintf("Saving results from pathway %s", i))
        writexl::write_xlsx(tmp.table, file.path(path.to.02.output, "pathway_results_GSEA", sprintf("enriched_pathway_%s.xlsx", i)))
      }
    }
