...
assign_class_for_Feature6 <- function(x, y, z){
  if (x == "T1" & y == "N0" && z == "M0"){
    output.class <- 1
  } else if (x == "T2" & y == "N0" && z == "M0"){
    output.class <- 2
  } else if((x == "T3" & y == "N0" & z == "M0") | (x %in% c("T1", "T2", "T3") & y == "N1" & z == "M0")){
    output.class <- 3
  } else if ((x == "T4" & y %in% c("N0", "N1") & z == "M0") | (x %in% c("T1", "T2", "T3", "T4") & y == "N2" & z == "M0")){
    output.class <- 4
  } else if ((y == "N3" &  z == "M0") | (x == "T4" & z == "M0")){
    output.class <- 5
  } else if (z == "M1"){
    output.class <- 6
  } else {
...
convert.feature.to.num <- function(featuredf){
  ##### convert Nx and Mx to N0 and M0, respectively. 
  featuredf <- featuredf %>% rowwise() %>%
    mutate(feature4 = ifelse(feature4 %in% c("Nx", "NX", "NO"), "N0", feature4)) %>%
    mutate(feature5 = ifelse(feature5 %in% c("Mx", "MX", "MO", "N0"), "M0", feature5)) 
  featuredf$feature6 <- unlist(lapply(seq(1, nrow(featuredf)), function(i){
    x <- featuredf[i, ][["feature3"]]
    y <- featuredf[i, ][["feature4"]]
    z <- featuredf[i, ][["feature5"]]
    return(assign_class_for_Feature6(x,y,z))
  }))
  
  ##### generate feature6 based on feature 3 4 5, do not use current feature 6.
...
##### check features
for (i in colnames(featuredf)){
  print(sprintf("Feature %s", i))
  print(unique(test.featuredf[[i]]))
}


##### check test features
for (i in colnames(test.featuredf)){
  print(sprintf("Feature %s", i))
  print(unique(test.featuredf[[i]]))
}
